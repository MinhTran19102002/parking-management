...
describe('Test API user', () => {
  ...
  describe('Test ham findByID', () => {
    test('Cha ve mot object', async () => {
      ...
      try {
        const findByID = await userService.findByID(data);
        expect(findByID.name).toBe('(intermediate value) is not a constructor');
      } catch (error) {
        expect(error.message).toBe('(intermediate value) is not a constructor');
      }
    });
  });
  ...
  describe('Test ham save_payment', () => {
    test('Cha ve mot object', async () => {
      try {
        ...
        expect(error.message).toBe('Thanh toan da bi huy');
      }
    });
  });
